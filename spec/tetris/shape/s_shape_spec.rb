  ...
  describe S_Shape do
    ...
    describe '#draw' do
      it 'draws blocks of the S_Shape' do
        s_shape = S_Shape.new window
        expect(s_shape.block_1).to receive(:draw)
        expect(s_shape.block_2).to receive(:draw)
        expect(s_shape.block_3).to receive(:draw)
        expect(s_shape.block_4).to receive(:draw)
        s_shape.draw
      end
    end
    ...
    describe '#move' do
      it 'moves itself down' do
        ...
        expect{s_shape.move('down')}.
          to change{s_shape.block_1.y}.by(1)
        expect{s_shape.move('down')}.
          to change{s_shape.block_2.y}.by(1)
        expect{s_shape.move('down')}.
          to change{s_shape.block_3.y}.by(1)
        expect{s_shape.move('down')}.
          to change{s_shape.block_4.y}.by(1)
      end
      ...
      it 'moves itself right' do
        ...
        expect{s_shape.move('right')}.
          to change{s_shape.block_1.x}.by(1)
        expect{s_shape.move('right')}.
          to change{s_shape.block_2.x}.by(1)
        expect{s_shape.move('right')}.
          to change{s_shape.block_3.x}.by(1)
        expect{s_shape.move('right')}.
          to change{s_shape.block_4.x}.by(1)
      end
